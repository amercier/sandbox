 ...
 */
/*global define */
define('jssl/core/Class', [], function() {
	
	var VALID_CLASS_NAME = /^[A-Z](_?[a-zA-Z0-9]+)*$/,
	    initializing = false,
	    fnTest = /xyz/.test(function(){ var xyz; }) ? /\b_super\b/ : /\w*/, // check if "function decompilation" works
	    Class;
	...
	function copyMethods(methods, target, _super, result) {
		...
		for(key in methods) {
			if(methods.hasOwnProperty(key)) {
				...
				target[key] = typeof methods[key] === "function"
						&& typeof _super[key] === "function"
						&& fnTest.test(methods[key]) ? superMethodWrapper(key, methods[key]) : methods[key];
	...
	Class.name = 'Class'; // IE, has no effect on other browsers
	...
	Class.extend = function extendClass(name, mixins, staticProperties, properties) {
		...
		childClass = Class.createClass(initializing, name);
	...
	Class.getMethods = function getMethods() {
		var parents   = this.getParents(),
		    methods   = this.parent ? this.parent.getMethods() : {},
