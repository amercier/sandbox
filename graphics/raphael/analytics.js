...
window.onload = function () {
  ...
  var container = 'analytics-container',
    ...
    bottomgutter = 20,
    topgutter = 20,
    colorhue = 0.57 || Math.random(),
    color = 'hsl(' + [colorhue, .7, .5] + ')',
  ...
  var path = r.path().attr({stroke: color, 'stroke-width': 1.5, 'stroke-linejoin': 'round'}),
    ...
    leave_timer,
    blanket = r.set();
  label.push(r.text(60, 12, '24 hits').attr(txt)).attr({fill: '#000'});
  label.push(r.text(60, 27, '22 September 2008').attr(txt1).attr({fill: color}));
  label.hide();
  var frame = r.popup(100, 100, label, 'right').attr({fill: '#fff', stroke: '#000', 'stroke-width': 1, 'fill-opacity': .7}).hide();
  ...
  for (var i = 0, ii = labels.length; i < ii; i++) {
    ...
    var dot = r.circle(x, y, 2).attr({fill: '#fff', stroke: color, 'stroke-width': 2});
